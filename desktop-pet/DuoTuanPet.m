...
static const NSTimeInterval kRestReminderInterval = 30.0 * 60.0;
static const NSTimeInterval kRestReminderMinVisibleSeconds = 4.0;
static const NSTimeInterval kRestReminderMaxVisibleSeconds = 6.0;
...
- (void)replaceImage:(NSImage *)image petName:(NSString *)petName;
@end
...
- (NSMenu *)makePetSwitchMenu;
- (void)switchPetFromMenu:(id)sender;
@end
...
- (void)replaceImage:(NSImage *)image petName:(NSString *)petName {
  ...
  _sourceFrameHeight = imageSize.height / (CGFloat)kRows;
  _frameIndex = 0;
  self.toolTip = petName.length > 0 ? petName : @"Codex pet";
  self.needsDisplay = YES;
  [self scheduleNextFrame];
}
...
- (NSMenu *)makeMenu {
  ...
  NSMenuItem *switchItem = [[NSMenuItem alloc] initWithTitle:@"切换形象" action:nil keyEquivalent:@""];
  switchItem.submenu = [(AppDelegate *)NSApp.delegate makePetSwitchMenu];
...
- (void)applicationDidFinishLaunching:(NSNotification *)notification {
  ...
  NSString *petSlug = [self valueAfter:@"--pet-slug" inArguments:arguments] ?: @"duotuan";
  _currentPetSlug = [petSlug copy];
  _currentPetName = [petName copy];
  ...
  _extensionPath = [[self valueAfter:@"--extension-path" inArguments:arguments] ?: @"" copy];
  _switchPetUriPrefix = [[self valueAfter:@"--switch-pet-uri-prefix" inArguments:arguments] ?: @"" copy];
...
- (NSMenu *)makePetSwitchMenu {
  NSMenu *menu = [[NSMenu alloc] initWithTitle:@"切换形象"];
  NSArray<NSDictionary *> *packages = [self availablePetPackages];
  if (packages.count == 0) {
    NSMenuItem *emptyItem = [[NSMenuItem alloc] initWithTitle:@"未找到可用形象" action:nil keyEquivalent:@""];
  ...
  for (NSDictionary *package in packages) {
    NSString *slug = [self stringValue:package[@"slug"]];
    NSString *displayName = [self stringValue:package[@"displayName"]];
    NSMenuItem *item = [[NSMenuItem alloc] initWithTitle:displayName.length > 0 ? displayName : slug
                                                  action:@selector(switchPetFromMenu:)
                                           keyEquivalent:@""];
    item.target = self;
    item.representedObject = package;
    item.state = [slug isEqualToString:_currentPetSlug] ? NSControlStateValueOn : NSControlStateValueOff;
    [menu addItem:item];
  }
  return menu;
}
...
- (void)switchPetFromMenu:(id)sender {
  ...
  NSDictionary *package = [(NSMenuItem *)sender representedObject];
  ...
  NSString *slug = [self stringValue:package[@"slug"]];
  NSString *displayName = [self stringValue:package[@"displayName"]];
  NSString *spritesheetPath = [self stringValue:package[@"spritesheetPath"]];
  if (slug.length == 0 || spritesheetPath.length == 0) {
    return;
  }

  NSImage *image = [[NSImage alloc] initWithContentsOfFile:spritesheetPath];
  if (!image) {
    [self showBubbleWithTitle:@"切换失败" body:@"这个形象的 spritesheet 读不到。"];
    return;
  }

  _currentPetSlug = [slug copy];
  _currentPetName = [(displayName.length > 0 ? displayName : slug) copy];
  [(PetView *)_window.contentView replaceImage:image petName:_currentPetName];
  [self showBubbleWithTitle:@"已切换形象" body:[NSString stringWithFormat:@"现在是 %@。", _currentPetName]];
  [self notifyExtensionOfPetSwitch:slug];
}
...
- (void)startRestReminderTimer {
  [_restReminderTimer invalidate];
  _restReminderTimer = [NSTimer scheduledTimerWithTimeInterval:kRestReminderInterval
                                                        target:self
                                                      selector:@selector(showRestReminderIfIdle:)
                                                      userInfo:nil
                                                       repeats:YES];
}

- (void)showRestReminderIfIdle:(NSTimer *)timer {
  if (_bubbleDisplayState != BubbleDisplayStateNone || !_window) {
    return;
  }
  [self showRestReminderBubbleWithMessage:[self randomRestReminderMessage]];
}

- (NSString *)randomRestReminderMessage {
  NSArray<NSString *> *messages = @[
    @"还在努力啊？真棒棒呢～",
    @"卷王你好呀～",
    @"认真你就输了哦～"
  ];
  NSUInteger index = (NSUInteger)arc4random_uniform((uint32_t)messages.count);
  return messages[index];
}

- (void)showRestReminderBubbleWithMessage:(NSString *)message {
  [_bubbleTimer invalidate];
  _bubbleTimer = nil;
  [_bubbleWindow close];
  _bubbleWindow = nil;
  ...
  NSView *content = [[NSView alloc] initWithFrame:NSMakeRect(0, 0, width, height)];
  content.wantsLayer = YES;
  content.layer.backgroundColor = [NSColor colorWithWhite:1.0 alpha:0.96].CGColor;
  content.layer.cornerRadius = 22;
  content.layer.masksToBounds = YES;

  NSTextField *label = [NSTextField labelWithString:message ?: @"休息一下吧。"];
  label.frame = NSMakeRect(18, 18, width - 36, 30);
  ...
  [bubble orderFrontRegardless];
  _bubbleWindow = bubble;
  _bubbleDisplayState = BubbleDisplayStatePassive;

  NSTimeInterval durationRange = kRestReminderMaxVisibleSeconds - kRestReminderMinVisibleSeconds;
  NSTimeInterval duration = kRestReminderMinVisibleSeconds +
    (NSTimeInterval)arc4random_uniform((uint32_t)(durationRange * 1000.0 + 1.0)) / 1000.0;
  _bubbleTimer = [NSTimer scheduledTimerWithTimeInterval:duration
                                                  target:self
                                                selector:@selector(collapseBubble:)
                                                userInfo:nil
                                                 repeats:NO];
...
- (NSArray<NSDictionary *> *)availablePetPackages {
  NSMutableArray<NSDictionary *> *packages = [NSMutableArray array];
  NSMutableSet<NSString *> *seenSlugs = [NSMutableSet set];
  NSMutableArray<NSString *> *roots = [NSMutableArray array];
  if (_codexHomePath.length > 0) {
    [roots addObject:[_codexHomePath stringByAppendingPathComponent:@"pets"]];
  }
  if (_extensionPath.length > 0) {
    [roots addObject:[_extensionPath stringByAppendingPathComponent:@"pets"]];
  }
  ...
  for (NSString *root in roots) {
    ...
    for (NSString *entry in entries) {
      if ([seenSlugs containsObject:entry]) {
        continue;
      }

      NSString *directory = [root stringByAppendingPathComponent:entry];
      NSDictionary *package = [self loadPetPackageAtDirectory:directory slug:entry];
      if (!package) {
        continue;
      }

      [packages addObject:package];
      [seenSlugs addObject:entry];
    }
  }

  [packages sortUsingComparator:^NSComparisonResult(NSDictionary *left, NSDictionary *right) {
    NSString *leftName = [self stringValue:left[@"displayName"]];
    NSString *rightName = [self stringValue:right[@"displayName"]];
    return [leftName localizedCaseInsensitiveCompare:rightName];
  }];
  return packages;
}

- (NSDictionary *)loadPetPackageAtDirectory:(NSString *)directory slug:(NSString *)slug {
  if (directory.length == 0 || slug.length == 0) {
    return nil;
  }

  NSString *petJsonPath = [directory stringByAppendingPathComponent:@"pet.json"];
  NSDictionary *metadata = nil;
  NSData *data = [NSData dataWithContentsOfFile:petJsonPath];
  if (data.length > 0) {
  ...
  NSString *displayName = [self stringValue:metadata[@"displayName"]];
  if (displayName.length == 0) {
    displayName = slug;
  }

  NSString *spritesheetRelativePath = [self stringValue:metadata[@"spritesheetPath"]];
  if (spritesheetRelativePath.length == 0) {
    spritesheetRelativePath = @"spritesheet.webp";
  }
  NSString *spritesheetPath = spritesheetRelativePath.isAbsolutePath
    ? spritesheetRelativePath
    : [directory stringByAppendingPathComponent:spritesheetRelativePath];

  if (![NSFileManager.defaultManager fileExistsAtPath:spritesheetPath]) {
    return nil;
  }
...
- (void)notifyExtensionOfPetSwitch:(NSString *)slug {
  if (_switchPetUriPrefix.length == 0 || slug.length == 0) {
    return;
  }

  NSString *encodedSlug = [slug stringByAddingPercentEncodingWithAllowedCharacters:NSCharacterSet.URLQueryAllowedCharacterSet];
  NSString *safeSlug = encodedSlug.length > 0 ? encodedSlug : slug;
  NSURL *url = [NSURL URLWithString:[_switchPetUriPrefix stringByAppendingString:safeSlug]];
  if (url) {
    [NSWorkspace.sharedWorkspace openURL:url];
  }
}
