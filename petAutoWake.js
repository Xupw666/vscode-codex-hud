...
const ANY_AVATAR_FALLBACK_PATTERN = /g=`(?:codex|custom:[^`]+)`,_=a\(`selected-avatar-id`,null\)/;
const ANY_CUSTOM_AVATAR_FALLBACK_PATTERN = /g=`custom:[^`]+`,_=a\(`selected-avatar-id`,null\)/;
...
function patchCodexAvatarSource(source, petId = DEFAULT_PET_ID) {
  ...
  if (!ANY_AVATAR_FALLBACK_PATTERN.test(source)) {
    return {
      source,
      changed: false,
  ...
  return {
    source: source.replace(ANY_AVATAR_FALLBACK_PATTERN, targetFallback),
    changed: true,
    alreadyPatched: false
  };
}
...
function unpatchCodexAvatarSource(source, petId = DEFAULT_PET_ID) {
  ...
  if (ANY_CUSTOM_AVATAR_FALLBACK_PATTERN.test(source)) {
    return {
      source: source.replace(ANY_CUSTOM_AVATAR_FALLBACK_PATTERN, defaultFallback),
      changed: true,
      alreadyPatched: true
    };
  }
