...
test("pet auto wake can switch an existing custom avatar fallback", () => {
  const source = "var h=r(),g=`custom:duotuan`,_=a(`selected-avatar-id`,null);";
  const result = patchCodexAvatarSource(source, "custom:newma");

  assert.equal(result.changed, true);
  assert.match(result.source, /g=`custom:newma`,_=a\(`selected-avatar-id`,null\)/);
});
...
test("pet auto wake disable restores any custom avatar fallback", () => {
  const source = "var h=r(),g=`custom:newma`,_=a(`selected-avatar-id`,null);";
  const result = unpatchCodexAvatarSource(source, "custom:duotuan");
